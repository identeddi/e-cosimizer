...
public class AccountService {
	...
	@POST
	@Path("/register")
	public String register(@QueryParam("email") String email, @QueryParam("firstName") String firstName,
		...
		LOG.info("email: " + email + " firstname" + firstName + " lastName: " + lastName + "password: " + password
	...
	@POST
	@Path("/login")
	public String registerget(@QueryParam("email") String email, @QueryParam("password") String password) {
		LOG.info("logged in - email: " + email + "password: " + password);
		return "successfully logged in";
	}
	/*    *//**
			 * Creates a new member from the values provided. Performs
			 * validation, and will return a JAX-RS response with either 200 ok,
	 ...
	 * 
	 * // Create an "ok" response builder = Response.ok().entity(member); }
	 * catch (ConstraintViolationException ce) { // Handle bean validation
	 * issues builder = createViolationResponse(ce.getConstraintViolations()); }
	 * catch (ValidationException e) { // Handle the unique constrain violation
	 ...
	 *//**
		 ...
		 * errors then it will throw a ConstraintValidationException with the
		 * set of the constraints violated.
		 * </p>
		 * <p>
		 * If the error is caused because an existing member with the same email
		 * is registered it throws a regular validation exception so that it can
	 ...
	 * (emailAlreadyExists(member.getEmail())) { throw new ValidationException(
	 * "Unique Email Violation"); } }
	 * 
	 *//**
		 * Creates a JAX-RS "Bad Request" response including a map of all
		 * violation fields, and their message. This can then be used by clients
		 * to show violations.
		 * 
		 * @param violations
