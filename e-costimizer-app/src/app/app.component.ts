...
export class MyApp {
  ...
  constructor(platform: Platform, statusBar: StatusBar, splashScreen: SplashScreen, private auth: AuthService, private menuService: MenuService) {
    platform.ready().then(() => {
    ...
    this.menuService.menuSubject.asObservable().subscribe(() => {
      this.measureItems = this.menuService.menuItems;
    })
  }
  ...
  openPage(page: PageInterface) {
    ...
    if (page.index) {
      this.nav.setRoot(page.component, { tabIndex: page.index, powerMeasureType: page.powerMeasureType }).catch(() => {
        console.log("Didn't set nav root");
      });
    } else if(page.powerMeasureType ){
      this.nav.setRoot(page.component, { powerMeasureType: page.powerMeasureType }).catch(() => {
        console.log("Didn't set nav root");
      });
    }else {
